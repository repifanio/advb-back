...
class SearchAllIndicatesBySector {
  public async run(sectorId: number):Promise<iIndicates[]> {
    ...
    SELECT i.indicate_id AS indicateId, i.description, c.company_id AS companyId, c.name FROM indicates i, companies c WHERE i.company_id = c.company_id and i.open_to_vote = true and i.sector_id = ? and c.accepted_to_vote = false;
  `, [sectorId]);
